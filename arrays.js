...
//   => ['banana', 'Bonobo]
function bWords(words) {return words.filter(word => word[0].toLowerCase() === 'b');
}
...
function findWordsStartingWith(words, letter) {
  return words
  .map((word, index) => (word.startsWith(letter) ? index : -1))
...
function smallestNItems(items, n) {
  ...
  const smallestItems = sortedItems.slice(0, n);
  return smallestItems.reverse();
}
